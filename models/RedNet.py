...
class RedNet(tf.keras.Model):
    ...
    def __call__(self):
        ...
        ################### Depth Branch ##############################
        #starter_block
        block0_depth = _start_block(depth_input, name='depth_conv1')
        depth_maxpool = tf.keras.layers.MaxPool2D(pool_size=3, strides=2, padding='same', name="depth_pool1_pool")(block0_depth)
        #block1
        block1_depth = _bottleneck_resblock(depth_maxpool, 256, resnet_strides[0], resnet_dilations[0], 'depth_conv2_block1', identity_connection=False)
        for i in range(2, blocks[0]+1):
            block1_depth = _bottleneck_resblock(block1_depth, 256, 1, 1, 'depth_conv2_block%d'%i)
        #block2
        block2_depth = _bottleneck_resblock(block1_depth, 512, resnet_strides[1], resnet_dilations[1], 'depth_conv3_block1', identity_connection=False)
        for i in range(2, blocks[1]+1):
            block2_depth = _bottleneck_resblock(block2_depth, 512, 1, 1, 'depth_conv3_block%d'%i)
        #block3
        block3_depth = _bottleneck_resblock(block2_depth, 1024, resnet_strides[2], resnet_dilations[2], 'depth_conv4_block1', identity_connection=False)
        for i in range(2, blocks[2]+1):
            block3_depth = _bottleneck_resblock(block3_depth, 1024, 1, 1, 'depth_conv4_block%d'%i)
        #block4
        block4_depth = _bottleneck_resblock(block3_depth, 2048, resnet_strides[3], resnet_dilations[3], 'depth_conv5_block1', identity_connection=False)
        for i in range(2, blocks[3]+1):
            block4_depth = _bottleneck_resblock(block4_depth, 2048, 1, 1, 'depth_conv5_block%d'%i)


        ################### RGB Branch ##############################
        #starter_block
        block0_rgb = _start_block(rgb_input, name='rgb_conv1')
        #block1
        ...
        block1_rgb = _bottleneck_resblock(rgb_maxpool, 256, resnet_strides[0], resnet_dilations[0], 'rgb_conv2_block1', identity_connection=False)
        for i in range(2, blocks[0]+1):
            block1_rgb = _bottleneck_resblock(block1_rgb, 256, 1, 1, 'rgb_conv2_block%d'%i)

        #block2
        block1_fused = tf.keras.layers.Add()([block1_rgb, block1_depth])
        block2_rgb = _bottleneck_resblock(block1_fused, 512, resnet_strides[1], resnet_dilations[1], 'rgb_conv3_block1', identity_connection=False)
        for i in range(2, blocks[1]+1):
            block2_rgb = _bottleneck_resblock(block2_rgb, 512, 1, 1, 'rgb_conv3_block%d'%i)

        #block3
        block2_fused = tf.keras.layers.Add()([block2_rgb, block2_depth])
        block3_rgb = _bottleneck_resblock(block2_fused, 1024, resnet_strides[2], resnet_dilations[2], 'rgb_conv4_block1', identity_connection=False)
        for i in range(2, blocks[2]+1):
            block3_rgb = _bottleneck_resblock(block3_rgb, 1024, 1, 1, 'rgb_conv4_block%d'%i)

        #block4
        block3_fused = tf.keras.layers.Add()([block3_rgb, block3_depth])
        block4_rgb = _bottleneck_resblock(block3_fused, 2048, resnet_strides[3], resnet_dilations[3], 'rgb_conv5_block1', identity_connection=False)
        for i in range(2, blocks[3]+1):
            block4_rgb = _bottleneck_resblock(block4_rgb, 2048, 1, 1, 'rgb_conv5_block%d'%i)

        block4_fused = tf.keras.layers.Add()([block4_rgb, block4_depth])

        ################### Decoder Branch ##############################
        agent0 =  _make_agant_layer(block0_fused, 64, "agent0")
        agent1 =  _make_agant_layer(block1_fused, 64, "agent1")
        agent2 =  _make_agant_layer(block2_fused, 128, "agent2")
        agent3 =  _make_agant_layer(block3_fused, 256, "agent3")
        agent4 =  _make_agant_layer(block4_fused, 512, "agent4")

        x = agent4
        trans1 = _make_transpose(x, 256, 6)
        x = tf.keras.layers.Add()([trans1, agent3])

        trans2 = _make_transpose(x, 128, 4)
        x = tf.keras.layers.Add()([trans2, agent2])

        trans3 = _make_transpose(x, 64, 3)
        x = tf.keras.layers.Add()([trans3, agent1])

        trans4 = _make_transpose(x, 64, 3)

        x = tf.keras.layers.Add()([trans4, agent0])

        trans5 = _make_transpose(x, 64, 3, upsample=False)
...
def _bottleneck_resblock(x, filters, stride, dilation_factor, name, identity_connection=True):
...
def _make_agant_layer(x, filters, name):
    layers = tf.keras.Sequential([
        tf.keras.layers.Conv2D(filters, kernel_size=1, strides=1, padding='same', use_bias=False),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.ReLU()
    ], name=name)
    return layers(x)

def _make_transpose(x, filters, nb_blocks, upsample=True):
    x1 = None
    if(not upsample):
        x1 = tf.keras.layers.Conv2D(filters, kernel_size=1, strides=1, padding='same', use_bias=False)(x)
        x1 = tf.keras.layers.BatchNormalization()(x1)
    else:
        x1 = tf.keras.layers.Conv2DTranspose(filters, kernel_size=3, strides=2, padding='same', use_bias=False)(x)
        x1 = tf.keras.layers.BatchNormalization()(x1)
    ...
    if(not upsample):
        ...
        x = tf.keras.layers.Conv2DTranspose(filters, kernel_size=3, strides=2, padding='same', use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization()(x)
...
def _load_pretrained(model) :
    ...
    for l in model.layers:
        ...
        if(layer_name.startswith(("rgb_", "depth_"))):
            ...
            if(not layer_name.startswith(("conv1", "pool1"))):
        ...
        # Initiate conv1 of the rgb branch. The conv1 in Depth branch is not initialize because the inuput there is 1 channel
        if(layer_name == "rgb_conv1_conv"):
            weight_layer = weight_model.get_layer("conv1_conv")
            l.set_weights(weight_layer.get_weights())
