...
class ESANet(tf.keras.Model):
    def __init__(self, num_classes=16, input_shape=(512,512,4)):
        ...
        self.conv_1 = tf.keras.Sequential([
                  tf.keras.layers.Conv2D(512, kernel_size=1, padding='same'),
                  tf.keras.layers.BatchNormalization(),
                  tf.keras.layers.ReLU(),
              ])
        self.conv_2 = tf.keras.Sequential([
                  tf.keras.layers.Conv2D(256, kernel_size=1, padding='same'),
                  tf.keras.layers.BatchNormalization(),
                  tf.keras.layers.ReLU(),
              ])
        self.conv_3 = tf.keras.Sequential([
                  tf.keras.layers.Conv2D(128, kernel_size=1, padding='same'),
                  tf.keras.layers.BatchNormalization(),
                  tf.keras.layers.ReLU(),
              ])
        ...
        # Classifier
        self.classifier = tf.keras.layers.Conv2D(num_classes, kernel_size=3, padding='same')
    ...
    def __call__(self):
        ...
        depth = tf.expand_dims(self.inputs[:,:,:,3], axis=-1)
        depth_input = tf.concat([depth, depth, depth], -1)
    ...
    def __init__(self, in_dim, out_dim, bins=(1, 2, 3, 6),
        ...
        for bin_size in bins:
            self.features.append(tf.keras.Sequential([
                ...
                tf.keras.layers.Conv2D(reduction_dim, kernel_size=1, padding='same'),
                tf.keras.layers.BatchNormalization(),
                activation
            ]))

        self.final_conv = tf.keras.Sequential([
                  tf.keras.layers.Conv2D(out_dim, kernel_size=1, padding='same'),
                  tf.keras.layers.BatchNormalization(),
                  activation,
              ])
...
class NonBottleneck1D(tf.keras.layers.Layer):
    ...
    def __init__(self, out_channels, stride=1, downsample=None, groups=1, base_width=None, dilation=1, norm_layer=None,
        ...
        self.conv3x1_1 = tf.keras.layers.Conv2D(out_channels, (3, 1), strides=(stride, 1), padding='same', use_bias=True)
        self.conv1x3_1 = tf.keras.layers.Conv2D(out_channels, (1, 3), strides=(1, stride), padding='same', use_bias=True)
        self.bn1 = tf.keras.layers.BatchNormalization(epsilon=1e-03)
        self.act = activation
        self.conv3x1_2 = tf.keras.layers.Conv2D(out_channels, (3, 1), padding='same', use_bias=True, dilation_rate=(dilation, 1))
        self.conv1x3_2 = tf.keras.layers.Conv2D(out_channels, (1, 3), padding='same', use_bias=True, dilation_rate=(1, dilation))
...
class LearnedUp(tf.keras.layers.Layer):
    def __init__(self, factor=2):
        ...
        self.depth_wise = tf.keras.layers.DepthwiseConv2D(kernel_size=3, kernel_initializer=initializer, padding='same')
...
class SqueezeAndExciteFusionAdd(tf.keras.layers.Layer):
    def __init__(self, C, reduction=16):
        ...
        self.se_rgb = tf.keras.Sequential([
            tf.keras.layers.GlobalAveragePooling2D(keepdims=True),
            tf.keras.layers.Conv2D(C // reduction, kernel_size=1, padding='same'),
            tf.keras.layers.ReLU(),
            tf.keras.layers.Conv2D(C, kernel_size=1, padding='same'),
