...
class CreateDataset_rgb() :
    ...
    def get(self):
        ...
        def preprocess(x, y):
            ...
            images.set_shape([self.IMAGE_SHAPE[0], self.IMAGE_SHAPE[1], 3])
        ...
        def preprocess_aug2(x, y):
            def f(x_path,y_path):
                ...
                # Apply image transformations
                # Random flip left and right.
                x = tf.image.stateless_random_flip_up_down(x, seed=s)
                y = tf.image.stateless_random_flip_up_down(y, seed=s)
                # Random crop
                crop = (self.IMAGE_SHAPE[0]// 4, self.IMAGE_SHAPE[1]//4)
                x = tf.image.stateless_random_crop(x, size=(self.IMAGE_SHAPE[0]-crop[0], self.IMAGE_SHAPE[1]-crop[1], 3), seed=s)
                y = tf.image.stateless_random_crop(y, size=(self.IMAGE_SHAPE[0]-crop[0], self.IMAGE_SHAPE[1]-crop[1], 1), seed=s)
            
                x = tf.image.resize(x, size=(self.IMAGE_SHAPE[0], self.IMAGE_SHAPE[1]), method='bilinear')
